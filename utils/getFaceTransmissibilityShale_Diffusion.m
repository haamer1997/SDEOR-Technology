function T_diff = getFaceTransmissibilityShale_Diffusion(G, rock, varargin)
%Compute face diffusion transmissibilities, accounting for input-specific multipliers
%
% SYNOPSIS:
...
%}
   ...
   % Hassan Edit: 
   T_diff = compute_onesided_trans(G, rock, varargin{:});

   T_diff = apply_onesided_multipliers(T_diff, mult);
   T_diff = apply_harmonic_reduction(T_diff, G);
   T_diff = apply_face_multipliers(T_diff, mult);
end
...
function T = compute_onesided_trans(G, rock, varargin)
   if has_cpgeometry(G)
      ...
      T = computeTransShale_Diffusion(G, rock, varargin{:}, ...
                       'K_system'       , 'loc_xyz', ...
...
%--------------------------------------------------------------------------
%Hassan Edit: modify harmonic reduction to match no. of columns in T_diff
function T = apply_harmonic_reduction(T, G)
   [xx, yy] = ndgrid(G.cells.faces(:,1),1:size(T,2));
    T = 1 ./ accumarray([xx(:) yy(:)], 1 ./ T(:));
    T(isinf(T)|isnan(T)) = 0; % adjust for single-phase diffusion
%   source: https://itectec.com/matlab/matlab-how-to-use-accumarray-over-a-matrix-avoiding-a-loop/
end
