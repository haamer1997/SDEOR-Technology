...
%% Diffusion Modeling
%MS Diffusion
fluidname = 'barnett3comps_nat_vars'; 
G.rock.shaleMechanisms.MS_Diffusion = 1;
G.rock.tau = 1.7;
...
arg_MS_Diffusion = {G, G.rock, flowfluid, fluid, 'water', false};
...
dd = computeNabla(modelDiagonalAD_MS_Diffusion.G, modelDiagonalAD_MS_Diffusion.G.rock);
modelDiagonalAD_MS_Diffusion.operators.dd = dd(modelDiagonalAD_MS_Diffusion.operators.internalConn);
modelDiagonalAD_MS_Diffusion.operators.g_gradz = modelDiagonalAD_MS_Diffusion.operators.Grad(G.cells.centroids(:,3))*9.81;
...
if (opt.shouldPlot)
    figure, 
    plotToolbar(G, states_MS_Diffusion)
    view(40,30);
    axis tight equal;
