function [Dg,Do] = MSdiffNatVars_Dynamic(fluidname,state) %dynamic mode implementation
%% Compute Averaged State Thermodynamical Properties
% Here, we preform arithamtic average for pressure, temperature, and
% component compositions for the entire cells
state.pressure = mean(state.pressure);
state.T = mean(state.T);
state.components = [mean(state.components),0]; %account for dummy component at the end
%% Set up Unit Cell
G = cartGrid([1, 1, 1]);
G = computeGeometry(G);
...
useAD = true;
statec = initCompositionalState(G, state.pressure,  state.T, [0.01, 0.01, 0.98], state.components, cmodel.EOSModel); %G, p, T, saturationVector, zi,eosModel
