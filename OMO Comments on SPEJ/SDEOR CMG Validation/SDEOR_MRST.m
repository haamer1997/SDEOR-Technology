%%
% This MATLAB script simualtes SDEOR base case (without NFs) to validate agaianst
...
G_matrix = tensorGrid(0:10:physdim(1), 0:5:physdim(2),z);
...
flowfluid.cR = 3e-6/psia;
flowfluid.pvMultR = @(p)(1 + flowfluid.cR .* (p-info.pressure));
...
case 'ProdBot_InjTop'
    % Producer
    W = verticalWell(W, G, G.rock, 10, 10, frac_z(2), ...
        'comp_i', [0.23, 0.76, 0.01],'Name', 'Prod_Bot', 'Val', pwf, 'sign', -1, 'Type', 'bhp','Radius', wellRadius); 
...
%         'comp_i', [0 0 1],'Name', 'Inj_Top', 'Val', pinj, 'sign', 1, 'Type', 'bhp','Radius', wellRadius); %control by injection pressure
    % Injector
    W = verticalWell(W, G, G.rock, 10, 10, frac_z(1), ...
...
if ~opt.shouldPlot
    ...
    fullFinalOut = [fpath, 'SDEOR_MRST.mat'];
    save(fullFinalOut,'ws','RF','schedule','-v7.3');
end
%% Spits out DI_DJ_DK for CMG Simulation
% DX=diff([0:10:physdim(1)])';
% DY=diff([0:5:physdim(2)])';
% DZ=diff(z)';
% fid = fopen('C:\Users\ahass16\Desktop\Research Work\mrst-2020a\modules\shale_chapter\OMO Comments on SPEJ\SDEOR CMG Validation\DI_DJ_DK.txt', 'w'); 
