...
G_matrix.rock=makeShaleRock(G_matrix,50*micro*darcy,0.07); %20*micro*darcy
...
case 'ProdBot_InjTop'
    % Producer
    W = verticalWell(W, G.Matrix, G.Matrix.rock, 19, 10, frac_z(2), ...
        'comp_i', [0.23, 0.76, 0.01],'Name', 'Prod_Bot', 'Val', pwf, 'sign', -1, 'Type', 'bhp','Radius', wellRadius); 
...
%         'comp_i', [0 0 1],'Name', 'Inj_Top', 'Val', pinj, 'sign', 1, 'Type', 'bhp','Radius', wellRadius); %control by injection pressure
    % Injector
    W = verticalWell(W, G.Matrix, G.Matrix.rock, 19, 10, frac_z(1), ...
...
if ~opt.shouldPlot
    ...
    fullFinalOut = [fpath, 'EOR_512NF_SubHorizontal_50microD_Gravity.mat'];
    save(fullFinalOut,'ws','G','schedule','-v7.3');
end