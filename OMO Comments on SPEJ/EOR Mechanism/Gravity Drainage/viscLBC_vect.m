function viscMix = viscLBC_vect(c, MW, TinK, Pci, Tci, Vci, zi)
...
%   Detailed explanation goes here
    ...
    sqrtMW = sqrt(MW);
%     mu0_mix = (([zi(:,1).*mu0_i(1), zi(:,2).*mu0_i(2)])*sqrtMW') ./ (zi*sqrtMW');
    mu0_mix = bsxfun(@times,zi,mu0_i)*sqrtMW' ./ (zi*sqrtMW');
...
%     viscMix = viscMix * 0.001; %convert viscosity from cp to Pa.s
    if (~isreal(viscMix))        
        fprintf('viscosity is not physical\n')
    end
end
