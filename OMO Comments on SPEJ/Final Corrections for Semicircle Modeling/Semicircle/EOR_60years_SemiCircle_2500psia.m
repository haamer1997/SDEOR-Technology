...
%% Basic Parameters
tol=1e-5;    
physdim = [200 100 80]*meter; %sets grid's physica 3 l dimensions (x,y,z)
...
nls.LinearSolver.replaceInf = true; nls.LinearSolver.replaceNaN = true;
nls.LinearSolver.maxIterations = 75;
nls.maxIterations = 30; nls.maxTimestepCuts = 10; nls.continueOnFailure = true;
disp(linsolve)
%% Schedule
wellRadius = 0.01;
W = [];
...
plotWell(G,W); 
dt = rampupTimesteps(totTime, 60*day, nSteps); %20*day
...
dt_equil = rampupTimesteps(60*day, 5*day, 7);
...
if ~opt.shouldPlot
    ...
    fullFinalOut = [fpath, 'EOR_60years_SemiCircle_2500psia.mat'];
