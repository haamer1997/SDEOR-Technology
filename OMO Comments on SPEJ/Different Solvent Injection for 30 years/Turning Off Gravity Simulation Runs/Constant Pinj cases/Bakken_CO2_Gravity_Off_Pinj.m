...
info.injection = [1 0 0 0 0 0 0]; %C1 injection
...
case 'ProdBot_InjTop'
    ...
    W = verticalWell(W, G, G.rock, 20, 10, frac_z(1), ...
        'comp_i', [0 0 1],'Name', 'Inj_Top', 'Val', rate, 'sign', 1, 'Type', 'bhp','Radius', wellRadius); %control by injection pressure
    W = verticalWell(W, G, G.rock, 20, 10, frac_z(1), ...
        'comp_i', [0 0 1],'Name', 'Inj_Top', 'Val', pinj, 'sign', 1, 'Type', 'bhp','Radius', wellRadius); %control by injection pressure
%     W = verticalWell(W, G, G.rock, 20, 10, frac_z(1), ...
%         'comp_i', [0 0 1],'Name', 'Inj_Top', 'Val', p3, 'sign', 1, 'Type', 'bhp','Radius', wellRadius); %control by injection pressure
%     W = verticalWell(W, G, G.rock, 20, 10, frac_z(1), ...
%         'comp_i', [0 0 1],'Name', 'Inj_Top', 'Val', p4, 'sign', 1, 'Type', 'bhp','Radius', wellRadius); %control by injection pressure
...
%         'comp_i', [0 0 1],'Name', 'Inj_Top', 'Val', rate, 'sign', 1, 'Type', 'rate','Radius', wellRadius); %control by injection rate
    W(1).components = info.initial;
    W(2).components = info.injection;
    W(3).components = info.injection;
%     W(4).components = info.injection;
%     W(5).components = info.injection;
otherwise
...
%% Create specific schedule for const Pinj
schedule = struct();
[Inj_1, Inj_2] = deal(W);

Inj_1(3).status = false;
Inj_2(2).status = false;


schedule.control = [struct('W', Inj_1);...  
                    struct('W', Inj_2)];   
                
control_id = [1*ones(26,1);2*ones(356,1)];
...
if ~opt.shouldPlot
    ...
    fullFinalOut = [fpath, 'Bakken_CO2_Gravity_Off_Pinj.mat'];
    save(fullFinalOut,'ws','RF','schedule','-v7.3');
end