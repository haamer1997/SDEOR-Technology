...
%                              chemical potential only in kg/(m^2.s)
    ...
    if isfield(model.G.rock, 'shaleMechanisms') && isfield(model.G.rock.shaleMechanisms,'MS_ECL_Diffusion')
        ...
        %(MS) diffusion matrix and the basic diffusion flux equation for 
        %combined chemical potential and gravity effects in kg/(m^2.s) at
        %dynamic mode
        RT = 8.314*temp(1);
        DgXGrad_g = 0; DoXGrad_o = 0;
        [Dg,Do] = MSdiffNatVars_Dynamic(model.G.rock.fluidname,state);
        const_g = s.faceAvg(rhoG.*yM{i}.*model.G.rock.poro.*sG./(model.G.rock.tau.*RT));
        const_o = s.faceAvg(rhoO.*xM{i}.*model.G.rock.poro.*sO./(model.G.rock.tau.*RT));
        for j = 1:ncomp
            DgXGrad_g = DgXGrad_g + Dg(i,j).*(s.Grad(RT.*log(f_V{i}))-compFluid.molarMass(i).*s.g_gradz);
            DoXGrad_o = DoXGrad_o + Do(i,j).*(s.Grad(RT.*log(f_L{i}))-compFluid.molarMass(i).*s.g_gradz);
        end
        eqs{i} = eqs{i} - s.Div(const_g.*DgXGrad_g) - s.Div(const_o.*DoXGrad_o);  
    end
    if isfield(model.G.rock, 'shaleMechanisms') && isfield(model.G.rock.shaleMechanisms,'MS_Diffusion')
        %Hassan edit to implement diffusion modeling using Maxwell-Stefan
        %(MS) diffusion matrix and the basic diffusion flux equation: 
        %J = (-phi.S/tau).pho.D.Grad(X)
        DgXyM = 0; DoXxM = 0;
        Dg = model.G.rock.Dg_MS; Do = model.G.rock.Do_MS;
        phiXsatXrho_tau_g = s.faceAvg(rhoG.*sG.*model.G.rock.poro./model.G.rock.tau);
        phiXsatXrho_tau_o = s.faceAvg(rhoO.*sO.*model.G.rock.poro./model.G.rock.tau);
        for j = 1:ncomp
            DgXyM = DgXyM + Dg(i,j).*s.Grad(yM{j});
            DoXxM = DoXxM + Do(i,j).*s.Grad(xM{j});
        end
        eqs{i} = eqs{i} - s.Div(phiXsatXrho_tau_g.*DgXyM) - s.Div(phiXsatXrho_tau_o.*DoXxM);  
    end
