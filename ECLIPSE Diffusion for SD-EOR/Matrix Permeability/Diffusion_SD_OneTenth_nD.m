...
G_matrix.rock=makeShaleRock(G_matrix,0.1*nano*darcy,0.07); %20*micro*darcy
...
pwf = 1000*psia;
pinj = 1600*psia;
casename = 'oil_1'; %'oil_1' %'bakken_light';
% pwf = 2500*psia;
% pinj = 3000*psia;
rate = 0.003277; %10,000 scf/day = 0.003277 m^3/s

fluidname = 'oil_1_modified'; %'bakken_light_1';
% diffusion
...
% G.rock.Di_o=G.rock.Di ./ 100; %diffusivity coefficients in oil phase for Bakken Light fluid
%G.rock.Di=[0.28,0.187,0.95]*10^-7; %diffusivity coefficients for oil_1
%fluid in oil phase
G.rock.Di=[2.8,1.87,0.95]*10^-10;%[0,0,0];%[2.8,1.87,0.95]*10^-8; %diffusivity coefficients for oil_1 fluid in gas phase
G.rock.Di_o = [2.8,1.87,0.95]*10^-12; %G.rock.Di ./ 100; %[2.8,1.87,0.95]*10^-5;%G.rock.Di ./ 100; %[0,0,0]; %G.rock.Di ./ 100;
G.rock.tau = 1.5;
[G.rock.Di,G.rock.Di_o] = HassanD_i_a(fluidname,G.rock.Di,G.rock.Di_o);

[fluid, info] = getShaleCompFluidCase(casename);
info.injection = [1 0 0]; %inject C1
...
modelSparseAD.operators.dd = dd(modelSparseAD.operators.internalConn);
modelSparseAD.operators.g_gradz = modelSparseAD.operators.Grad(G.cells.centroids(:,3))*9.81; 
...
linsolve = selectLinearSolverAD(modelDiagonalAD,'useAMGCL',false,'useCPR',false); %GMRES Solver
% linsolve = selectLinearSolverAD(modelDiagonalAD,'useAMGCL',true,'useCPR',true);
% linsolve = selectLinearSolverAD(modelSparseAD,'useAMGCL',false,'useCPR',true,'useAGMG',true); %AGMG Solver
disp(linsolve)
...
% [ws, states, reports] = simulateScheduleAD(state, modelSparseAD, schedule,'nonlinearsolver', nls, 'Verbose', true); %Sparse solver
...
if ~opt.shouldPlot
    ...
    fullFinalOut = [fpath, 'Diffusion_SD_OneTenth_nD.mat'];
    save(fullFinalOut,'ws','RF','Np','G','schedule');
end