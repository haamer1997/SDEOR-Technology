function [G,fracplanes]=EDFMshalegrid(G,fracplanes,varargin)
...
for j=1:length(fracturelist)
    ...
    if G.rock.shaleMechanisms.Diffusion
        FracGrid_new.(fieldname).rock.Dg = ones(FracGrid_new.(fieldname).cells.num,1)*fracplanes(i).Dg; %Hassan Edit: add fracture diffusion coeff. to matrix coeff.
        FracGrid_new.(fieldname).rock.Do = ones(FracGrid_new.(fieldname).cells.num,1)*fracplanes(i).Do;
    end
...
G=assembleShaleGlobalGrid(Gtemp);
