...
%     startIdx = numel(G.nnc.cells(:,1))+1; %this will be the index of first projected cell
    ...
    if G.rock.shaleMechanisms.Diffusion
        %Hassan Edit: define T_diff for pNNC
        p.Trans_diff_gas = zeros(nRows4pNNC,size(G.rock.Dg,2));
        p.Trans_diff_oil = zeros(nRows4pNNC,size(G.rock.Do,2));
    end
...
%             localIdxNonEqui = 3*(i-1)+selectedDir;
            ...
            %Hassan Edit:
            if G.rock.shaleMechanisms.Diffusion
            [p.Type{localIdxEqui}, p.Area(localIdxEqui), p.Trans(localIdxEqui),p.Trans_diff_gas(localIdxEqui,:),p.Trans_diff_oil(localIdxEqui,:), p.Dir(localIdxEqui)] = ...
                    computeNNCprojArea_n_Trans_Diff(G,p.MMneighs(localIdxEqui,:),p.MFcells(localIdxEqui,:),mcells(i,:),fracMatAreas(i));
            else
...
%                     faceSelected(3) = 1;
    ...
    %Hassan Edit: append pMat-F T_diff to G
    if G.rock.shaleMechanisms.Diffusion
        G.nnc.Tdiff_gas = [G.nnc.Tdiff_gas; p.Trans_diff_gas];
        G.nnc.Tdiff_oil = [G.nnc.Tdiff_oil; p.Trans_diff_oil];
    end
...
function [pType, pArea, pTrans,pTrans_diff_gas,pTrans_diff_oil, projDir] = computeNNCprojArea_n_Trans_Diff(G,pMMneighs,pMFcells,mcells,fracMatArea)
    if (pMFcells(1)~=0)
        %if matrix host cell is on the boundary, projection is
        %unncessary. Initialize all nnc arrays correctly in this case
        pType = 'fracpmat';
...
%        K_pMF = harmmean(G.rock.perm(pMFcells));
        ...
        pTrans = K_pMF*pArea/dist_pMF; %pTrans is T_pM-F
        projDir = find(G.faces.normals(globalIdx4pMM,:));
        %Hassan Edit: Compute T_diff for pMat-Frac 
        Dg_pMF = length(pMFcells)./sum(1./G.rock.Dg(pMFcells,:));
        Do_pMF = length(pMFcells)./sum(1./G.rock.Do(pMFcells,:));
        pTrans_diff_gas = Dg_pMF.*pArea./dist_pMF; 
        pTrans_diff_oil = Do_pMF.*pArea./dist_pMF;
    else
        %it means pM cell is on the boundary of the domain, and we don't
        %need any projection
        pType = 'ProjMatrix outside';
        ...
        projDir = 0;
        %Hassan Edit:
        pTrans_diff_gas = zeros(1,size(G.rock.Dg,2)); 
        pTrans_diff_oil = zeros(1,size(G.rock.Do,2));
    end 
...
function [p] = prepareFaceCalcs(p,localIdx,G,mcells,fracMatArea,currFaceID,projectionCells)                  
    ...
    %Hassan Edit: T_diff outputs
        if G.rock.shaleMechanisms.Diffusion
        [p.Type{localIdx}, p.Area(localIdx), p.Trans(localIdx),p.Trans_diff_gas(localIdx,:),p.Trans_diff_oil(localIdx,:), p.Dir(localIdx)] = ...
                computeNNCprojArea_n_Trans_Diff(G,p.MMneighs(localIdx,:),p.MFcells(localIdx,:),mcells,fracMatArea);
    else
