function T_diff = computeTransShale_Diffusion(G, rock, varargin)
%Compute transmissibilities for diffusion modeling.
%
% SYNOPSIS:
...
%}
   ...
   opt = struct('verbose'        , false, ...
                ...
                'K_system'       , 'xyz', ...
                'cellCenters'    , []   , ...
                'fixNegative'    , false, ... %Hassan Edit: do not fix negative T_diff (MS matrix has negative numbers)
                'cellFaceCenters', []   , ...
                'phase'          , varargin{2});
   ...
   switch lower(opt.K_system)
      case 'xyz',     T_diff = htrans_xyz  (G, rock, opt);
      case 'loc_xyz', T_diff = htrans_local(G, rock, opt);
   ...
   if opt.fixNegative
      T_diff = handle_negative_trans(T_diff, opt);
   end
   T_diff = handle_ntg(T_diff, rock, G);
...
function T = htrans_xyz(G, rock, opt) 
   ...
   switch(opt.phase)
       case 'gas'
         Dg = G.rock.Dg;
         T = T .* Dg(cellNo,:); %map from cell to half-face multicomponent diffusion    
       case 'oil'
         Do = G.rock.Do;
         T = T .* Do(cellNo,:); %map from cell to half-face multicomponent diffusion    
   end
%    T = T ./ sum(C .* C, 2);
end
...
function T = htrans_local(G, rock, opt) %Hassan Edit: I have not modified this yet
