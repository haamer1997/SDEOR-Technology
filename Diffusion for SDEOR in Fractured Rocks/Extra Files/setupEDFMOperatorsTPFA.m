...
%Hassan Edit: add T_diff field in opt variable
% opt = struct('deck', [], 'neighbors', [], 'trans', [], 'porv', []);
opt = struct('deck', [], 'neighbors', [], 'trans', [],'trans_diff_gas', [],'trans_diff_oil', [], 'porv', []);
opt = merge_options(opt, varargin{:});

%Hassan Edit: Define T_diff variables
T = opt.trans;
Tdiff_gas = opt.trans_diff_gas; Tdiff_oil = opt.trans_diff_oil; 
N = opt.neighbors;
...
if isempty(T)
   ...
   if G.rock.shaleMechanisms.Diffusion
       %Hassan Edit: Compute standard T_diff and EDFM NNC
       Tdiff_gas = getFaceTransmissibilityShale_Diffusion(G, rock, 'phase','gas');
       Tdiff_oil = getFaceTransmissibilityShale_Diffusion(G, rock, 'phase','oil');

       Tdiff_gas=Tdiff_gas.*transmult; 
       s.Tdiff_gas_all = Tdiff_gas; % T_diff_gas_all will not contain nnc transmissibilities!!!!!
       Tdiff_gas=[Tdiff_gas;G.nnc.Tdiff_gas]; % modified line!!!!!!!!!!!!!!!!
       Tdiff_gas = Tdiff_gas(intInx,:);

       Tdiff_oil=Tdiff_oil.*transmult; 
       s.Tdiff_oil_all = Tdiff_oil; % T_diff_gas_all will not contain nnc transmissibilities!!!!!
       Tdiff_oil=[Tdiff_oil;G.nnc.Tdiff_oil]; % modified line!!!!!!!!!!!!!!!!
       Tdiff_oil = Tdiff_oil(intInx,:);
   end
else
   s.T_all = T;
   T = T(intInx);
