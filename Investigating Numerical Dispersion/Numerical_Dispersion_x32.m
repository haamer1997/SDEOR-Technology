...
%% Load necessary modules, etcQ_gs
...
[G_matrix,frac_ids,well_ids]=explicitStencil(physdim, 'aperture',w_f,...
     'numStages',1,'fracSpacing', 200, 'fracHalfLength'  , x_f,      ...
     'nxRefine',32,'tipNY',2,'ny',8, 'nz',8, 'gridType','geomspacing'); 
...
%Rock Properties
matrix_perm = 1*milli*darcy; %100*micro
matrix_poro= 0.063;
...
casename = 'barnett3comps'; %'barnett3comps';
fluidname = 'barnett3comps_modified'; 
pwf = 500*psia;
pinj = 8000*psia;
G2=G;
...
dd = computeNabla(modelSparseAD.G, modelSparseAD.G.rock);
...
%% Calculate Activity-Corrected Diffusion Coefficients
% Da_V = zeros(1,ncomp); Da_L = zeros(1,ncomp);
% df_dy = [7.00e6,1.60e7,1.10e7]; %Meisam's values
% for i = 1:ncomp
%     Da_V(i) = fug{i,2}*G.rock.Di(i)/(state0.y(i)*df_dy(i)); %
%     Da_L(i) = fug{i,1}*G.rock.Di_o(i)/state0.x(i); %we did not have df_L/dx 
% end
% G.rock.Di = Da_V;
% G.rock.Di_o = Da_L;
%% Pick linear solver
...
% [ws1, states1, reports1] = simulateScheduleAD(state1, modelSparseAD, schedule1, 'nonlinearsolver', nls,'Verbose', true);
% [ws1, states1, reports1] = simulateScheduleAD(state1, modelDiagonalAD, schedule1, 'nonlinearsolver', nls,'Verbose', true);
[ws1, states1, reports1] = simulateScheduleAD(state1, modelMexDiagonalAD, schedule1, 'nonlinearsolver', nls,'Verbose', true);
% [ws1, states1, reports1] = simulateScheduleAD(state1, modelSparseAD, schedule1,'Verbose', true); %direct solver
%% plotting
if opt.shouldPlot
    figure, 
...
if ~opt.shouldPlot
    ...
    fullFinalOut = [fpath, 'Numerical_Dispersion_x32.mat'];
    save(fullFinalOut,'ws1','Q_gs','G','schedule1');
end
