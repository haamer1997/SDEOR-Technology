...
set1 = Field(DFN('dim',3,'n',800,'dir',0,'ddir',0,'minl',0.3,...
           'mu',5,'maxl',10,'bbx',[tol4domain,tol4domain,tol4domain,physdim(1)-tol4domain,physdim(2)-tol4domain,physdim(3)-tol4domain],'dip',0,'ddip',0,...
           'shape','s'),'Poly'); 
set2 = Field(DFN('dim',3,'n',800,'dir',90,'ddir',-1e9,'minl',0.6,...
            'mu',5,'maxl',10,'bbx',[tol4domain,tol4domain,tol4domain,physdim(1)-tol4domain,physdim(2)-tol4domain,physdim(3)-tol4domain],'dip',90,'ddip',-1e9,...
            'shape','s'),'Poly');  
set3 = Field(DFN('dim',3,'n',448,'dir',0,'ddir',-1e9,'minl',0.6,...
            'mu',5,'maxl',10,'bbx',[tol4domain,tol4domain,tol4domain,physdim(1)-tol4domain,physdim(2)-tol4domain,physdim(3)-tol4domain],'dip',0,'ddip',-1e9,...
            'shape','s'),'Poly'); %'l','q',4
...
for i=1:numel(fracSet)
    ...
    fracplanes(i).aperture = 0.01*ft;% 100*nano*meter; %1*micro*meter;
...
if ~opt.shouldPlot
    ...
    fullFinalOut = [fpath, 'Percolation_1024NF_Producer.mat'];
    save(fullFinalOut,'ws','RF','states','G','schedule','frac_intensity');
end